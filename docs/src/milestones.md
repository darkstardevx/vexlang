...
typed AST span attachment remains a follow-up refinement. Declaration-site
labels are available for selected semantic errors. Analyzer-native multi-error
collection has started for undefined-variable and undefined-function diagnostics
in `check`, alongside conservative line-oriented parse recovery; broader
